;(function () {
  ...
  function testName(title, pid, id) {
    var child = getChild(pid)
    for (var i = 0; i < child.length; i++) {
      if (child[i].title == title && child[i].id != id) {
        return true
      }
    }
    return false
  ...
  function createTreeMenu(pid, level, open) {
    ...
    for (var i = 0; i < nowData.length; i++) {
      ...
      inner +=
        '<li class="' + (isOpen(nowData[i].id, open) ? 'open' : '') + '">'
      inner +=
        '<p data-id="' +
        nowData[i].id +
        '" style="padding-left:' +
        ...
        (hasChild ? 'has-child' : '') +
        ' ' +
        (nowId == nowData[i].id ? 'active' : '') +
        '"><span>' +
        nowData[i].title +
        '</span></p>'
      if (hasChild) {
        //如果当前项有子项，就在这里再生成一个子级的ul
  ...
  function createBreadNav() {
    ...
    for (var i = 0; i < allParent.length; i++) {
      inner +=
        '<a data-id="' + allParent[i].id + '">' + allParent[i].title + '</a>'
    }
  ...
  function createFolders() {
    ...
    for (var i = 0; i < child.length; i++) {
      ...
      inner +=
        '<input type="checkbox" ' + (child[i].checked ? 'checked' : '') + ' />'
  ...
  contextmenuBtn[2].onclick = function () {
    var folderItem = folders.querySelector(
      '.folder-item[data-id="' + active_id + '"]'
    )
    contextmenu.style.display = 'none'
    rename(folderItem)
  }
