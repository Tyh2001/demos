...
<template>
  <div>
    <ul>
      <!-- 不同状态，在显示与隐藏之间切换 -->
      <li v-for="(fruit,index) in fruitList" :key="index" v-show="fruit.isShow">
        <span v-bind:class="{checked:fruit.isChecked}" class="no-checked" @click="toggle(index)"></span>
        <span v-bind:class="{'checked-text':fruit.isChecked}" class="txt">{{fruit.text}}</span>
